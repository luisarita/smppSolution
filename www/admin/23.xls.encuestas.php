...
if (isset($_POST['desde']) && isset($_POST['hasta'])) {
    ...
    $sql = sprintf("SELECT r.id, r.nombre, p.fecha, p.numero, p.mensaje FROM encuestas r, encuestas_participantes p WHERE p.fecha>=%s AND p.fecha<=%s AND p.idEncuesta=r.id ORDER BY r.nombre, p.fecha", GetSQLValueString($desde, "date"), GetSQLValueString($hasta, "date"));
    $rs = mysql_query($sql, $conexion) or die(register_mysql_error("XE001", mysql_error()));
    ...
    $path = "encuestas.xlsx";
    $fullpath = tempnam(sys_get_temp_dir(), 'encuestas');
    ...
    <body>
        <form id="frmGenerar" name="frmGenerar" method="post">
            <table width="220px" cellpadding="0" cellspacing="0">
                <tr><th colspan="2">Consolidado de Encuestas</th></tr>
