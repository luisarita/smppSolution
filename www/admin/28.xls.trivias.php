...
if (isset($_POST['desde']) && isset($_POST['hasta'])) {
    ...
    $sql = sprintf("SELECT r.id, r.nombre, p.fecha, p.numero, '' AS mensaje FROM trivias r, trivias_participantes p WHERE p.fecha>=%s AND p.fecha<=%s AND p.idTrivia=r.id ORDER BY r.nombre, p.fecha", GetSQLValueString($desde, "date"), GetSQLValueString($hasta, "date"));
    $rs = mysql_query($sql, $conexion) or die(register_mysql_error("XV001", mysql_error()));
    ...
    $path = "trivias.xlsx";
    $fullpath = tempnam(sys_get_temp_dir(), 'trivias');
...
<html xmlns="http://www.w3.org/1999/xhtml">
    <head>
        ...
        <title>Consolidado de Trivias</title>
    ...
    <body>
        <form id="frmGenerar" name="frmGenerar" method="post">
            <table width="220px" cellpadding="0" cellspacing="0">
                <tr><th colspan="2">Consolidado de Trivias</th></tr>
