...
if (isset($_POST['desde']) && isset($_POST['hasta'])) {
    ...
    $condicion_numero = (isset($_POST['numero_salida']) && $_POST['numero_salida'] != '') ? sprintf(" AND telefon_destino_de_respu=%s", $_POST['numero_salida']) : "";
    $sql = sprintf("SELECT telefon_destino_de_respu AS nombre, fecha_de_recepcion AS fecha, telefono_origen_de_respue AS numero, datos_recibidos AS mensaje FROM comunicacion_recibida_tabla WHERE fecha_de_recepcion>=%s AND fecha_de_recepcion<=%s %s UNION SELECT telefon_destino_de_respu AS nombre, fecha_de_recepcion AS fecha, telefono_origen_de_respue AS numero, datos_recibidos AS mensaje FROM comunicacion_recibida_tabla_bck WHERE fecha_de_recepcion>=%s AND fecha_de_recepcion<=%s %s ORDER BY nombre, fecha;", GetSQLValueString($desde, "date"), GetSQLValueString($hasta, "date"), $condicion_numero, GetSQLValueString($desde, "date"), GetSQLValueString($hasta, "date"), $condicion_numero);
...
<html xmlns="http://www.w3.org/1999/xhtml">
    <head>
        ...
        <title>Consolidado de Recepción</title>
    ...
    <body>
        <form id="frmGenerar" name="frmGenerar" method="post">
            <table width="220px" cellpadding="0" cellspacing="0">
                <tr><th colspan="2">Consolidado de Recepcion</th></tr>
                ...
                <tr>
                    <td>Número:</td>
                    <td><select name="numero_salida" id="numero_salida" class="textbox-small">
