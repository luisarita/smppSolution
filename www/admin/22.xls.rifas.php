...
if (isset($_POST['dateFrom']) && isset ($_POST['dateTo'])){
    ...
    $sql = sprintf("SELECT r.id, r.nombre, p.fecha, p.numero, p.mensaje, p.variable1 FROM rifas r, rifas_participantes p WHERE p.fecha>=%s AND p.fecha<=%s AND p.idRifa=r.id AND (-1=%s OR r.id=%s) ORDER BY r.id, p.fecha", 
        GetSQLValueString($dateFrom, "date"), 
        GetSQLValueString($dateTo, "date"),
