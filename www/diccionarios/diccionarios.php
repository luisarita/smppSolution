...
class diccionarios {
 ...
 private $templateHTML     = "diccionarios/html.html";
 private $templateSorteo   = "diccionarios/sorteo.html";
 
 private $title       = "Diccionarios";
 private $table       = "diccionarios";
 private $tableDetail = "(SELECT lp.id, lm.idDiccionario, lp.fecha, lp.estado, lp.numero, lp.mensaje, lp.contestado, lp.seleccionado FROM diccionarios_participantes lp, diccionarios_palabras lm WHERE lm.id=lp.idPalabra)";
 private $tableUpdate = "diccionarios_participantes";
 private $sessionVar  = "idDiccionario";
 ...
 function printOpciones           (){
  ...
  $SQL = sprintf("SELECT id, clave, valor FROM diccionarios_palabras WHERE %s=%s ORDER BY clave;", $this->sessionVar, GetSQLValueString($_SESSION[$this->sessionVar], "int"));
 ...
 function reiniciar               (){
  ...
  mysql_query($sql, $conexion) or die(register_mysql_error("R0003", mysql_error()));
 ...
 function eliminar                (){
  ...
  foreach ( $ids as $id => $value ){
   $sql = sprintf("DELETE FROM diccionarios_palabras WHERE id=%s AND %s=%s;", GetSQLValueString($value, "int"), $this->sessionVar, GetSQLValueString($_SESSION[$this->sessionVar], "int"));
 ...
 function agregar                 (){
  ...
  $clave = $_POST['clave'];
  $valor = $_POST['valor'];
  $sql = sprintf("INSERT INTO diccionarios_palabras (idDiccionario, clave, valor) VALUES (%s, %s, %s);", GetSQLValueString($_SESSION[$this->sessionVar], "int"), GetSQLValueString($clave, "text"), GetSQLValueString($valor, "text") );
  mysql_query($sql, $conexion) or die(register_mysql_error("E0001", mysql_error()));
